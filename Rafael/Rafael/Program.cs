...
app.MapPost("/api/folha/cadastrar", ([FromBody] Folha folha, [FromServices] AppDataContext ctx) =>
{
    ...
    if(folha.SalarioBruto <= 1903.98)
        folha.ImpostoIRRF = 0;
    if(folha.SalarioBruto <= 2826.48)
        folha.ImpostoIRRF = (folha.SalarioBruto * 0.75) - 142.80;
    if(folha.ImpostoIRRF <= 3751.05)
        folha.ImpostoIRRF = (folha.SalarioBruto * .15) - 354.80;
    if(folha.ImpostoIRRF <= 4664.68)
        folha.ImpostoIRRF = (folha.SalarioBruto * .255) - 636.13;
    else
    ...
    folha.SalarioLiquido = folha.SalarioBruto - folha.ImpostoIRRF - folha.ImpostoINSS - folha.ImpostoFGTS;
